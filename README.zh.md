...
- **始终置顶**：浮窗每 2 秒复查一次自己的层级，并在每次前台窗口变化、显示模式变化后复查；当有窗口盖住面板一半以上时会把自己抬回最顶层（临时弹出的气泡/菜单不会触发）。抬升过程不会抢走前台应用的焦点。
...
- **独占全屏（游戏里的「全屏」模式）下无法叠加显示。** Windows 不会在独占全屏的交换链上合成任何其它窗口，普通应用做不到——浮窗检测到这种情况时会在 `%APPDATA%\deskpulse\diag.log` 里写一行说明。「无边框/窗口化全屏」则没有这个限制。
